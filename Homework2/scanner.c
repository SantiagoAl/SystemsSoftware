...
// This is my final solution for programming assignment #2. I implemented it
// using a linked list, to store the tokens
...
list *createNode(int data, char *buffer, int value)
{
...
list *insertToken(list *tail, int data, char *buffer, int value)
{
...
// so I can easily set the head pointer in main() back to NULL by calling this
// function
list *destroyLinkedList(list *head)
{
    list *current = head;
    list *temp;
    ...
    // Return NULL which will assign the head to 'NULL' back in main
    return NULL;
}
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf("Please provide an input file through the command line\n");
        return 1;
    }
    ...
    // Open the file passed to the program. One to read from and insert into the
    // lexeme list and the other to print out the source program
    filePointer = fopen(argv[1], "r");
    printFile = fopen(argv[1], "r");

    // Check if the file was successfully opened, if not, return 1 and terminate
    // the program
    if (filePointer == NULL || printFile == NULL)
    {
        printf("Failed to open input file\n");
        return 1;
    }

    // This will kick of the printing of the source program to the screen
    ch = fgetc(printFile);
    printf("Source Program: \n");

    // The main loop for printing out the source program
    while (ch != EOF)
    {
            printf("%c", ch);
            ch = fgetc(printFile);
    ...
    while (ch != EOF)
    {
        ...
        if (isalpha(ch))
        {
            ...
            while (isalpha(ch = fgetc(filePointer)) || isdigit(ch))
            {
                if (index > 10)
                {
            ...
            // Print out out the string to the Lexeme list and its associated
            // token type
            printf("%-13s", variableString);
            printf("%d\n", reservedWord);
        ...
        else if (isdigit(ch))
        {
            ...
            int nextDigit = 0, placeValue = 1, index = 0;

            while (isdigit(ch = fgetc(filePointer)))
            {
                if (placeValue > 4)
                {
            ...
            // Print the value and associated token type to the Lexeme Table
            printf("%-13d", value);
            printf("%d\n", numbersym);
        ...
        else
        {
            ...
            switch(specialSymbol)
            {
                ...
                case 10:
                    ...
                    if (nextPrint)
                    {
                        char character[3] = {prevCh, ch, '\0'};
                        printf("%-13s", character);
                        printf("%d\n", type);
                    }
        ...
        // This is used initially, whent he linked list is first created
        if (head == NULL)
            head = tail;
    }

    printf("\n");
    // Now, print out the lexeme list
    printf("Lexeme List:\n");
    printList(head);

    // Remember to free all of the memory associated with this linked list
    head = destroyLinkedList(head);
    tail = NULL; // Alternatively, 'tail == head', since head is now NULL
